...
function Gestures(listener) {
  ...
  this.install = function(canvas) {
    ...
    // Desktop (Chrome)
    if (!isAndroid) {
      canvas.onmousedown = gestureStartHandler
      canvas.onmousemove = gestureMoveHandler
      canvas.onmouseup   = gestureEndHandler
    }

    // Android (Chrome)
    if (isAndroid) {
      canvas.addEventListener('touchstart', gestureAndroidStartHandler, false)
...
//    console.log(vector)
  ...
  function gesture3(a, b, c) {
    ...
    if (a === Direction.opposite(c)) return "U"//U(a, b, c)
...
var Direction = new Object() //todo do same as factory?
{
  Direction.N  = 1
  Direction.NE = 2
  Direction.E  = 3
  ...
  Direction.diagonal = function(dir) {
    return !Direction.pure(dir)
  }

  Direction.pure = function(dir) {
    var dirs = [Direction.N, Direction.S, Direction.W, Direction.E]
